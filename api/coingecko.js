...
export default async function handler(request) {
  ...
  if (!coinId && !contractAddress) {
    return new Response(
      JSON.stringify({ error: 'Missing coinId or contract parameter' }),
      {
        status: 400,
  ...
  try {
    ...
    if (contractAddress) {
      ...
      url = `https://api.coingecko.com/api/v3/coins/${platform}/contract/${contractAddress}`;
    } else {
      // Query by coin ID
